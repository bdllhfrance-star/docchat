...
- **Surface centrale avant le chat** : aucun bouton ni dropzone documentaire.
  ...
  paramètres utiles : contexte, sortie maximale et niveau de raisonnement. Le
  symbole utilise sans modification l'asset RGB officiel Google
  `Gemini_SparkIcon_4C` conservé localement.
...
| [x] | CHAT-01 | RAG-03 | Backend/RAG | `POST /api/chat`, validations avant stream, contexte dynamique, prompt fondé uniquement sur les documents, refus sans contexte et UI message stream sourcé sont implémentés. En production, `hello` répond naturellement sans retrieval ni source, tandis qu'une question sur la fixture TXT passe par le RAG et retourne les quatre faits exacts avec citations. |
| [~] | UI-05 | UI-04, CHAT-01 | UI | Chat relié à `/api/chat` avec `useChat`, historique `sessionStorage` isolé par batch, annulation et affichage progressif. Pendant la recherche et le streaming, le logo Smartly.ai utilise une animation organique de pop/halo avec un libellé d'étape ; `prefers-reduced-motion` la neutralise. Le composer ne s'active que lorsque tous les documents sont `ready`. |
| [x] | UI-06 | UI-05 | UI | Markdown rendu sans HTML brut, réponse assistant directement sur la surface et citations intégrées par réponse. Chaque citation ouvre un popover ancré près du marqueur, au-dessus ou en dessous selon l'espace disponible, sans allonger la réponse ni provoquer un scroll ; fermeture extérieure et `Escape` sont gérés. Fichier, emplacement et extrait restent visibles, tandis que les scores RRF/cosinus et la liste globale de candidats restent internes. |
...
| [x] | BON-05 | GATE-01, SET-05 | QA | Le parcours réel sur les PDF français et arabe a été exécuté sur l'URL stable : upload, extraction, indexation, réponses dans les langues attendues et citations aux pages 4/8 et 2/5. Les preuves sont enregistrées dans `docs/EVALUATION_RESULTS.md`. |
| [x] | BON-06 | BON-02, BON-05 | QA/RAG | Cinq questions réelles, dont un cas absent, ont été exécutées en production. Les cinq cas passent ; réponses, sources, refus, latences et scoring QA sont enregistrés dans `docs/EVALUATION_RESULTS.md`. |
...
| [x] | FIN-03 | GATE-02 | Documentation | Le README documente l'architecture globale, les trois flux principaux avec diagrammes Mermaid, l'API, les variables, les index Atlas, les limites et compromis serverless, la sécurité, les logs, le choix MongoDB, l'absence motivée de LangChain/LlamaIndex, le chunking/retrieval, les tests, l'évaluation, le lancement local et le déploiement. |
...
| 2026-09-03 | Lorsqu'une demande générale est redirigée hors du périmètre documentaire, faire produire à Gemini un marqueur de présentation masqué puis afficher trois boutons allowlistés vers les domaines officiels ChatGPT, Claude et Gemini. Une simple mention de ces produits dans un document ne déclenche aucun bouton. |
| 2026-09-03 | Si Gemini formule clairement une redirection mais omet son marqueur, reconnaître uniquement la combinaison stricte « limite Smartly.ai + recommandation générale + trois assistants attendus », puis construire les liens depuis l'allowlist locale. Les simples mentions documentaires restent du texte normal. |
| 2026-09-03 | Accélérer uniquement les premières demandes externes à très forte confiance (résultat sportif, météo, actualité, cours/taux live) en sautant embeddings et Atlas. Toute référence documentaire explicite, tout historique ou toute ambiguïté garde le RAG. Le routeur fournit seulement un hint ; Gemini rédige toujours la réponse. |
...
| 2026-09-03 | Tracer la fin réelle de chaque génération avec `finishReason`, compteurs de tokens et durée, distinguer les annulations client, et signaler dans l'interface toute réponse terminée autrement que par `stop` avec une régénération simple. Aucun contenu utilisateur n'entre dans ces logs. |
| 2026-09-03 | Remplacer les pastilles génériques des redirections externes par les marques officielles : logo produit ChatGPT observé sur `chatgpt.com`, Claude Spark Clay du press kit Anthropic et Gemini Spark RGB déjà validé. Les marques restent secondaires face à Smartly.ai et servent uniquement leurs liens officiels. |
